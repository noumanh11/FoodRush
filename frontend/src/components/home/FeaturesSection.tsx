...
export default function FeaturesSection() {
 ...
 <h2 className="mb-6 font-display text-4xl font-extrabold tracking-tight text-foreground transition-colors sm:text-5xl">
 Everything you need to{' '}
 <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-400 to-amber-300">order smarter</span>
 </h2>
 <p className="mx-auto max-w-2xl text-lg leading-relaxed text-muted-foreground transition-colors ">
 ...
 >
 <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-border bg-secondary text-2xl shadow-inner transition-colors " style={{ animationDelay: `${i * 0.2}s`}}>
 {stat.emoji}
 </div>
 <div>
 <p className="mb-1 text-sm font-medium uppercase tracking-widest text-muted-foreground transition-colors ">{stat.label}</p>
 <p className="flex items-center gap-2 font-display text-xl font-bold text-foreground transition-colors ">
 ...
 >
            <div className="relative h-56 overflow-hidden">
              ...
              <div className={`absolute top-4 left-4 w-12 h-12 rounded-2xl bg-gradient-to-br ${feature.color} backdrop-blur-md border border-white/20 flex items-center justify-center shadow-xl`}>
                <feature.icon size={24} className={feature.iconColor} />
              </div>
            </div>
            <div className="p-6 relative">
              <div className="absolute -top-12 right-6 flex h-16 w-16 items-center justify-center rounded-2xl border border-border bg-card/90 text-3xl shadow-xl shadow-black/5 dark:shadow-black/20 transition-transform duration-500 group-hover:-rotate-3">
                {feature.emoji}
              </div>
              <h3 className="mb-3 font-display text-2xl font-bold text-foreground transition-colors group-hover:text-brand-400">
 {feature.title}
 </h3>
 <p className="text-base leading-relaxed text-muted-foreground transition-colors ">{feature.description}</p>
 </div>
 </div>
 ))}
 </div>
